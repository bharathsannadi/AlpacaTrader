...
| 1 | **Backtest harness v2** — ✅ FREE-PATH DONE (`57bac3e`). 🟡 **PAID PATH = Polygon.io (user decision 2026-05-17, over Databento — recurring-cost trade-off noted & accepted).** Scope: 3-yr lookback, quote-snapshots-at-signal-times → needs **Options Developer ($79) + Stocks Starter ($29) = ~$108/mo** for real NBBO (Options *Starter* has no quotes). Build: `scripts/polygon_data.py` adapter, swap bt_v2 data layer. ⚠️ Earlier key = 403 'plan doesn't include' (ENTITLEMENT, not the current outage) — paid tiers must be ACTIVE. **Blocked on: Polygon service back UP + correct tiers active + key in .env → 'polygon ready'.** | ~6-9 build | 🟡 decided (Polygon); awaiting service+tiers+key | §🆕-P0-A below |
