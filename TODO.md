...
| 1 | **Backtest harness v2** — ✅ **FREE-PATH DELIVERED 2026-05-17** (`scripts/backtest_v2.py`, commit `57bac3e`). 60d/BS/VIX-IV, real evaluators, exit sweep, IVR buckets, walk-forward. **VERDICT: aggregate PF 0.63 NEGATIVE — but vwap_momentum PF 1.75 (edge) vs trend_cont PF 0.49 (75% of book = the bleed).** Paid 3-yr path still pending for go-live magnitudes. | ~16 | ✅ FREE PATH DONE / paid path → user data decision | §🆕-P0-A below |
...
| **17** | **🎯 Disable/hard-gate `trend_cont` signal class** — backtest 57bac3e: trend_cont PF 0.49, −1692% over 404 trades = the entire bleed; vwap_momentum PF 1.75 is the edge. HIGHEST-VALUE change in the codebase, $0, evidence-based (404-sample/6-sym/OOS-consistent), reversible via config flag. Restrict live to vwap_momentum (+gap_fade marginal). | ~1 | **NEXT — backtest-justified** | §🆕-P1-K below |
