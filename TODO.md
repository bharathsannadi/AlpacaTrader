...
| **CR-4** | **🟠 Refresh path blocks the hub + double work** — `account_value()` network read + fresh `RiskBrain` + route every symbol on every refresh AND on cached tab-open; plus `_annotate_kb` runs twice (re-scores stocks needlessly). Read equity from `state` snapshot; re-score only option rows after liquidity. | `app.py:_refresh_screener_bg` | 🟡 Partial 2026-06-04 — equity now read from `state` snapshot first (no blocking network read on hub); double-annotate stock re-score still TODO |
