...
| **2S-B** | **Fix the spread-data harness** — S2 failed from sparse short-leg OHLC / pricing-bar mismatch, NOT from spreads being bad. Build a correct 2-leg spread pricer (contemporaneous bars, liquid OTM strike selection, fallback). **Architectural reference (2026-05-19):** [`sirnfs/OptionSuite`](External_Projects/ANALYSIS_OptionSuite.md) — borrow patterns: PutVertical-as-object with leg-match invariants at constructor, primitive-level unit tests, event-driven tick→signal flow. Evaluate iVolatility data ($50-200/mo) vs Polygon Options Advanced ($199 NBBO) — iVolatility may be the better spend for chain depth. NOT wholesale code adoption. | ⬜ TODO | a clean S2 measurement (win% sane, n not collapsed); primitive unit-tests pass |
...
## 🆕 KB-COMPLY — KB Compliance Gaps (audited 2026-05-21)

Full audit of `daily_trader.py` + `spy_auto_trader.py` against all 25 KB sections + Appendix.
All items target the **Connors RSI(2) daily strategy** (`daily_trader.py`).
...
| **RC-1** | **Concentration / correlation cap** — the screener + autonomous lanes each only tracked their OWN held set, so together they built a 20+ same-direction LONG book that a single risk-off day (12/15 names down) stopped out 0W/20L for −$2,549 | THE failure mode of the 2026-06-05 down-day; KB-5 ("max 3 correlated") was never enforced across lanes | ✅ Done 2026-06-05 (commit 37eb5be) — `config.MAX_PORTFOLIO_POSITIONS=12` + `_portfolio_position_count()` (sums equity+option, -1→fail-safe); both auto-exec lanes gate before the entry loop + decrement a per-cycle slot counter on fill; 5 tests. **Next:** sector/beta sub-limits (no sector map yet — raw count is the first cut); make the daily-loss breaker also throttle on deep *unrealized* drawdown, not just realized |

### 🔴 Code-review findings (2026-06-04, merged-picks/sizing diff)

| ID | Finding | File | Status |
|----|---------|------|--------|
| **CR-1** | **🔴 Spread sizing defeats the $600 cap** — `qty = ceiling // net_debit` but the BTO pays the FULL long premium, so a debit spread's long leg can deploy multiples of $600 (e.g. net $1, long $5 → qty 6 → $3,000 long outlay). Ceiling check only validates net per-contract. Cap qty on the long-leg gross, or size by long premium. | `screener_executor.py:~539` | ✅ Fixed 2026-06-04 — size + ceiling on gross long outlay (`atm_mid*100`); `total_risk` reports net-debit max-loss |
| **CR-2** | **🟠 Transient equity=0 silently disables ALL auto-exec** — `_build_picks` sets `rb=None` when `account_value()` returns 0 (API hiccup / client None) → every pick routes "skip" → zero trades, no error. Fall back to last-known equity or the legacy lists. | `app.py:_build_picks` | ✅ Fixed 2026-06-04 — equity falls back to `state["account_value"]`; display-only only when both 0 |
| **CR-3** | **🟠 `_execPick` false "not executable"** — picks table renders 20 rows but `_scrOptRows`/`_scrDtRows` are sliced to 15; a pick ranked 16–20 → `findIndex` -1 → misleading toast on a valid BUY. Dispatch by the pick's own sym/payload, not an index into a truncated array. | `static/main.js:_execPick` | ✅ Fixed 2026-06-04 — `_renderOpt/DtTable` store the FULL rows so `_execPick` maps picks ranked >15 |
