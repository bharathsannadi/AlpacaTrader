...
A Flask + SocketIO web dashboard for automated options day-trading using the [Alpaca](https://alpaca.markets) API. Trades SPY, AMZN, GOOG, MSFT, NVDA, and META options with an all-day session combining ORB + VWAP momentum + trend continuation + mean reversion strategies, an LLM-powered bull/bear debate gate (28-book knowledge base), ChromaDB trade memory, news pre-filter, and full position persistence with two-way Alpaca reconcile across restarts.
